...
export function LocalLoginPage() {
  ...
  const { refresh, mfaRequired } = useAuth();
  ...
  const doMfaSkip = () =>
    wrap(async () => {
      const res = await api.post<LocalLoginResponse>('/auth/local/mfa/skip');
      // Prevents the app gate from bouncing back to the setup screen.
      sessionStorage.setItem('thoth-mfa-skipped', '1');
      await advance(res);
    });
          ...
          {step === 'mfa_setup_required' && (
            <Stack align="center">
              <Text size="sm" ta="center">
                {mfaRequired
                  ? 'Set up the required MFA. Scan the QR code with your authenticator app (TOTP) and confirm the code, or register a passkey.'
                  : 'Set up MFA (recommended). Scan the QR code with your authenticator app (TOTP) and confirm the code, or register a passkey.'}
              </Text>
              ...
              {!mfaRequired && (
                <Button variant="subtle" color="gray" fullWidth loading={busy} onClick={doMfaSkip}>
                  Skip for now (set up later in Account)
                </Button>
              )}
            </Stack>
          )}
