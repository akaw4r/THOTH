...
function Protected({ children }: { children: React.ReactNode }) {
  ...
  // Users without MFA are sent to the setup screen. When MFA is optional
  // (MFA_REQUIRED=false), the screen offers "Skip for now", which sets the
  // session flag below and lets them proceed; enrollment stays available in
  // Account settings.
  const mfaSkipped = sessionStorage.getItem('thoth-mfa-skipped') === '1';
  if (!user.mfaEnrolled && (mfaRequired || !mfaSkipped)) {
