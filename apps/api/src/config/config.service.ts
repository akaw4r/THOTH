...
const envSchema = z.object({
  ...
  LOCAL_ADMIN_ENABLED: z
    ...
    .default('true')
    .transform((v) => v === 'true'),
  // Require MFA enrollment before accessing the app. Default false: the user
  // may skip enrollment on first login and set it up later in Account.
  // Set true to enforce MFA for everyone (local admin and Google users).
  MFA_REQUIRED: z
    .enum(['true', 'false'])
    .default('false')
    .transform((v) => v === 'true'),
...
export class ConfigService {
  ...
  /** When true, MFA enrollment is enforced before any authenticated route. */
  get mfaRequired(): boolean {
    return this.env.MFA_REQUIRED;
  }
