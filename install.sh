...
# Unique compose project name per install: prevents two THOTH checkouts on the
# same machine from sharing containers/volumes (which causes Postgres password
# mismatches against a volume initialized by the other install).
COMPOSE_PROJECT_NAME="$(env_get COMPOSE_PROJECT_NAME)"
[ -n "$COMPOSE_PROJECT_NAME" ] || COMPOSE_PROJECT_NAME="thoth-$(openssl rand -hex 3)"

# MFA policy: false (default) lets users skip enrollment on first login and set
# it up later in Account settings; true enforces it. Preserved across re-runs.
MFA_REQUIRED="$(env_get MFA_REQUIRED)"
MFA_REQUIRED="${MFA_REQUIRED:-false}"

# true if something is already listening on the given local TCP port
port_in_use() {
  (exec 3<>"/dev/tcp/127.0.0.1/$1") 2>/dev/null
}

# first free port at or above the given one (gives up after 100 tries)
next_free_port() {
  local p="$1"
...
HTTP_PORT_DEFAULT="${HTTP_PORT_DEFAULT:-8080}"
while :; do
  ask HTTP_PORT "Local HTTP port" "$HTTP_PORT_DEFAULT"
  case "$HTTP_PORT" in (*[!0-9]*|'') die "Invalid port: $HTTP_PORT";; esac
  [ "$HTTP_PORT" -ge 1 ] && [ "$HTTP_PORT" -le 65535 ] || die "Invalid port: $HTTP_PORT"
  if port_in_use "$HTTP_PORT"; then
    warn "Port $HTTP_PORT is already in use by another process."
    if ask_yn "Use it anyway? (No = pick a different port)"; then
      break
    fi
    HTTP_PORT_DEFAULT="$(next_free_port $((HTTP_PORT + 1)))"
    info "Suggestion: port $HTTP_PORT_DEFAULT is free."
  else
    break
  fi
done
...
if ask_yn "Will you use Google sign-in?"; then
  ...
  info "No Google: access will be through the local admin (/auth/local route)."
fi
echo
...
# --- Application ---
# Compose project name (isolates this install's containers/volumes)
...
# Account); true = enforced for everyone.
