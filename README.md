...
- **Authentication** — optional Google OIDC + local break-glass admin with mandatory MFA (TOTP or WebAuthn passkey)
...
Open the **unlisted** route `http://localhost:8080/auth/local`. On first access the system **forces a password change and MFA enrollment** (TOTP via QR code or WebAuthn passkey).
